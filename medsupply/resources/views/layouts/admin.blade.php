...
[data-theme="dark"] {
  ...
  --mint: rgba(60,191,153,0.15);
  --lavender: rgba(139,127,212,0.15);
  --peach: rgba(224,112,80,0.15);
  --border: rgba(255,255,255,0.07);
  --text: #e8f0fe;
  --text2: #8ba3c7;
  --text3: #556280;
  --sidebar-bg: #131d2e;
  --topbar-bg: rgba(19,29,46,0.95);
  --card-bg: #1a2336;
...
.nav-badge.green { background: var(--mint-dark); }
...
.user-card:hover { background: var(--blue-soft); }
.user-avatar { width: 34px; height: 34px; border-radius: 9px; background: linear-gradient(135deg, var(--blue), var(--lavender-dark)); display: flex; align-items: center; justify-content: center; color: white; font-weight: 700; font-size: 13px; flex-shrink: 0; }
.user-name { font-size: 0.82rem; font-weight: 600; color: var(--text); }
.user-role { font-size: 0.7rem; color: var(--text3); }
.main { margin-left: var(--sidebar-w); flex: 1; display: flex; flex-direction: column; min-height: 100vh; }
.topbar { background: var(--topbar-bg); backdrop-filter: blur(12px); border-bottom: 1px solid var(--border); padding: 0 1.8rem; height: 62px; display: flex; align-items: center; justify-content: space-between; position: sticky; top: 0; z-index: 99; transition: background 0.3s; }
...
.search-box:focus-within { border-color: var(--blue); background: var(--card-bg); box-shadow: 0 0 0 3px rgba(91,155,213,0.1); width: 250px; }
...
.notif-dropdown { display: none; position: absolute; top: calc(100% + 10px); right: 0; width: 320px; background: var(--card-bg); border: 1px solid var(--border); border-radius: 16px; box-shadow: 0 12px 40px rgba(0,0,0,0.15); z-index: 999; overflow: hidden; animation: fadeUp 0.2s ease; }
...
.notif-empty { padding: 24px; text-align: center; color: var(--text3); font-size: 0.82rem; }
.content { padding: 1.8rem; flex: 1; }
.kpi-card { background: var(--card-bg); border-radius: var(--radius); padding: 1.3rem; box-shadow: var(--shadow); position: relative; overflow: hidden; animation: fadeUp 0.5s cubic-bezier(0.22,1,0.36,1) both; transition: transform 0.25s, box-shadow 0.25s; border: 1px solid var(--border); height: 100%; }
.kpi-card:hover { transform: translateY(-3px); box-shadow: 0 8px 32px rgba(91,155,213,0.14); }
.kpi-blob { position: absolute; top: -20px; right: -20px; width: 80px; height: 80px; border-radius: 50%; opacity: 0.08; }
...
.kpi-value { font-family: 'Fraunces', serif; font-size: 1.8rem; font-weight: 600; color: var(--text); line-height: 1; margin-bottom: 6px; }
...
.ms-table th { font-size: 0.7rem; font-weight: 700; color: var(--text3); text-transform: uppercase; letter-spacing: 0.5px; padding: 8px 12px; border-bottom: 1px solid var(--border); white-space: nowrap; }
...
.pill-chef { background: var(--mint); color: var(--mint-dark); }
.pill-supplier { background: var(--lavender); color: var(--lavender-dark); }
.pill-active { background: var(--mint); color: var(--mint-dark); }
.pill-inactive { background: var(--peach); color: var(--peach-dark); }
.pill-pending { background: #fff3cd; color: #856404; }
.pill-verified { background: var(--mint); color: var(--mint-dark); }
...
<nav class="sidebar">
  ...
  <a class="nav-item {{ request()->is('admin/hospitals*') ? 'active' : '' }}" href="/admin/hospitals">
    <i class="fa-solid fa-hospital"></i> Hôpitaux
  </a>
  <a class="nav-item {{ request()->is('admin/suppliers*') ? 'active' : '' }}" href="/admin/suppliers">
    <i class="fa-solid fa-truck-medical"></i> Fournisseurs
    <span class="nav-badge red">3</span>
  </a>

  <div class="sidebar-section">Opérations</div>
  <a class="nav-item {{ request()->is('admin/orders*') ? 'active' : '' }}" href="/admin/orders">
    <i class="fa-solid fa-file-invoice"></i> Commandes
    <span class="nav-badge">12</span>
  </a>

  <div class="sidebar-section">Système</div>
  <a class="nav-item {{ request()->is('admin/ai-assistant*') ? 'active' : '' }}" href="{{ route('admin.ai-assistant') }}">
    <i class="fa-solid fa-robot"></i> Assistant IA
  </a>
  <a class="nav-item {{ request()->is('admin/analytics*') ? 'active' : '' }}" href="{{ route('admin.analytics') }}">
    <i class="fa-solid fa-chart-line"></i> Analytics
  </a>
  <a class="nav-item {{ request()->is('admin/audit-logs*') ? 'active' : '' }}" href="{{ route('admin.audit-logs') }}">
    <i class="fa-solid fa-shield-halved"></i> Logs d'audit
  </a>
  <a class="nav-item {{ request()->is('admin/settings*') ? 'active' : '' }}" href="{{ route('admin.settings') }}">
...
<div class="main">
  <div class="topbar">
    <div class="topbar-title">
      ...
      <p>@yield('page-subtitle', 'MedSupply · Administration')</p>
    </div>
    <div class="topbar-right">
      ...
      <div class="notif-wrapper">
        <div class="topbar-icon-btn" id="notifBtn" onclick="toggleNotif()">
        ...
        <div class="notif-dropdown" id="notifDropdown">
          ...
          <div class="notif-item">
            <div class="notif-icon" style="background:var(--blue-soft);color:var(--blue-dark)"><i class="fa-solid fa-hospital"></i></div>
            <div>
              <div class="notif-text">Nouvel hôpital ajouté</div>
          ...
          <div class="notif-item">
            <div class="notif-icon" style="background:var(--mint);color:var(--mint-dark)"><i class="fa-solid fa-user-plus"></i></div>
            <div>
              <div class="notif-text">Nouvel utilisateur créé</div>
              <div class="notif-time">il y a 2h</div>
            </div>
          </div>
        </div>
      </div>
      <div class="topbar-icon-btn" id="themeBtn" onclick="toggleTheme()" title="Mode sombre">
